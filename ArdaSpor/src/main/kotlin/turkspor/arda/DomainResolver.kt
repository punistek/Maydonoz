...
class DomainResolver(private val preferences: SharedPreferences) {
    companion object {
        const val GATEWAY = "https://www.ardasporgiris.site/"
        const val BOOTSTRAP = "https://ardaspor30.top/"
    ...
    suspend fun resolve(force: Boolean = false): SiteSnapshot = mutex.withLock {
        ...
        suspend fun verify(url: String): SiteSnapshot? {
            return try {
                ...
                if (!SportsParser.isSource(response.text)) return null
                var items = SportsParser.channels(response.text, finalUrl)
                SportsParser.channelEndpoint(response.text)?.let { endpoint ->
                    try {
                        val data = app.get(endpoint, referer = finalUrl, headers = mapOf("User-Agent" to UA), timeout = 8)
                        val remoteItems = if (data.code == 200) SportsParser.channels(data.text, finalUrl) else emptyList()
                        if (remoteItems.isNotEmpty()) items = remoteItems
                ...
                SiteSnapshot(finalUrl, items, System.currentTimeMillis()).also {
                    ...
                    SportsParser.gatewayTargets(response.text,finalUrl).firstOrNull { link -> link != finalUrl && SportsParser.siteUrl(link) != null }?.let { next ->
                        preferences.edit().putString("nextDomain",next).apply()
                    }
                }
        ...
        try {
            ...
            if (response.code == 200) {
                ...
                if (root.optInt("schemaVersion") == 1) {
                    val urls = root.optJSONObject("sources")?.optJSONObject("ardaspor")?.optJSONArray("candidates")
        ...
        SportsParser.siteUrl(preferences.getString("nextDomain", "").orEmpty())?.let(candidates::add)
        candidates.add(BOOTSTRAP)
        candidates.add("https://www.atomsportv513.top/")
        ...
        throw ErrorLoadingException("ArdaSpor kaynağına ulaşılamadı. Eklenti ayarlarından Domaini yenile'yi deneyin. ${errors.lastOrNull().orEmpty()}")
    }
    ...
    suspend fun setManual(value: String): SiteSnapshot {
        ...
        val url = SportsParser.siteUrl(value) ?: throw ErrorLoadingException("Geçerli bir HTTPS ArdaSpor adresi girin.")
