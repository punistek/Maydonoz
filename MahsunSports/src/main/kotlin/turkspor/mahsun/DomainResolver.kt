...
class DomainResolver(private val preferences: SharedPreferences) {
    ...
    suspend fun resolve(force: Boolean = false): SiteSnapshot = mutex.withLock {
        ...
        suspend fun verify(url: String): SiteSnapshot? {
            return try {
                ...
                val dataUrl = SportsParser.dataScript(response.text, finalUrl) ?: return null
                val data = app.get(dataUrl, referer = finalUrl, headers = mapOf("User-Agent" to UA), timeout = 10)
                if (data.code != 200) return null
                val items = SportsParser.channels(data.text, finalUrl)
        ...
        try {
            ...
            if (response.code == 200) {
                ...
                if (root.optInt("schemaVersion") == 1) {
                    val urls = root.optJSONObject("sources")?.optJSONObject("mahsunsports")?.optJSONArray("candidates")
        ...
        throw ErrorLoadingException("MahsunSports kaynağına ulaşılamadı. Eklenti ayarlarından Domaini yenile'yi deneyin. ${errors.lastOrNull().orEmpty()}")
    }
    ...
    suspend fun setManual(value: String): SiteSnapshot {
        ...
        val url = SportsParser.siteUrl(value) ?: throw ErrorLoadingException("Geçerli bir HTTPS MahsunSports adresi girin.")
