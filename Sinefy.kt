...
class Sinefy : MainAPI() {
    ...
    private fun pagedUrl(base: String, page: Int): String {
        ...
        return when {
            cleanBase.contains("/gozat/") -> "$cleanBase?page=$page"
            Regex("/page/\\d+$").containsMatchIn(cleanBase) ->
                cleanBase.replace(Regex("/page/\\d+$"), "/page/$page")
            Regex("/dizi-izle/[^/]+/\\d+$").containsMatchIn(cleanBase) ->
                cleanBase.replace(Regex("/\\d+$"), "/$page")
            else -> if (page <= 1) base else cleanBase
        }
    }
